    ...
    def drive(self, distance):
        if self.gallons_left <= 0:
            ...
            if fuel_needed <= self.gallons_left:
                ...
                print(f'You went {distance} miles and have {self.gallons_left:.1f} gallons of fuel left.')
            else:
                distance_possible = self.mpg * self.gallons_left
                self.gallons_left = 0
                self.odometer += distance_possible
                print(f'You only went {distance_possible:.1f} miles and then ran out of gas.')
